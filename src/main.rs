...
#[derive(Parser)]
#[command(name = "forge3d", about = "Render a JSON scene to a 3D MP4, or check/preview it first.")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn run_validate(scene: &Path) -> Result<(), String> {
    match forge3d::validate_scene_file(scene) {
        Ok(()) => {
...
fn run_frame(scene: &Path, out: &Path, t: f32) -> Result<(), String> {
    ...
    forge3d::render_frame_png(scene, out, t).map_err(|e| e.to_string())?;
...
fn run_render(scene: &Path, out: &Path) -> Result<(), String> {
    let started = Instant::now();
    forge3d::render_video(scene, out, |done, total| {
...
fn run_storyboard(scene: &Path, out: &Path, frames: u32) -> Result<(), String> {
    let started = Instant::now();
    forge3d::render_storyboard_png(scene, out, frames).map_err(|e| e.to_string())?;
    println!("wrote {} ({:.2}s)", out.display(), started.elapsed().as_secs_f32());
    Ok(())
}
